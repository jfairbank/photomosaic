export default class Resizer {
  ...
  resizeWidthRGB(buffer) {
    ...
    const nextLineOffsetOriginalWidth = this.originalWidthMultipliedByChannels - 2;
  ...
  resizeWidthInterpolatedRGB(buffer) {
    ...
    for (targetPosition = 0; weight < 1 / 3; targetPosition += 3, weight += ratioWeight) {
      for (
  ...
  resizeWidthRGBA(buffer) {
    const ratioWeight = this.ratioWeightWidthPass;
    const ratioWeightDivisor = 1 / ratioWeight;
    const nextLineOffsetOriginalWidth = this.originalWidthMultipliedByChannels - 3;
  ...
  resizeWidthInterpolatedRGBA(buffer) {
    ...
    // Handle for only one interpolation input being valid for start calculation:
    for (targetPosition = 0; weight < 1 / 3; targetPosition += 4, weight += ratioWeight) {
      for (
        finalOffset = targetPosition, pixelOffset = 0;
        finalOffset < this.widthPassResultSize;
        pixelOffset += this.originalWidthMultipliedByChannels,
